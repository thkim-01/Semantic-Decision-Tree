...
class SDTLearner:
    ...
    def __init__(self, max_depth: int = 10, min_samples_split: int = 5,
                 min_samples_leaf: int = 2, verbose: bool = True):
        """
        Args:
            max_depth: 트리의 최대 깊이
    ...
    def _find_best_refinement(self, node: TreeNode) -> tuple:
        """
        정보 이득이 가장 높은 refinement 찾기
        
        Returns:
            (best_refinement, best_information_gain)
        """
        # 가능한 refinement 생성
        ...
        for refinement in candidate_refinements:
            gain = self._calculate_information_gain(node, refinement)
    ...
    def _calculate_information_gain(self, node: TreeNode, refinement: Refinement) -> float:
        """정보 이득 계산"""
        # 분할 수행
        left_instances, right_instances = self.refinement_operator.apply_refinement(
        ...
        information_gain = node.entropy - weighted_child_entropy
